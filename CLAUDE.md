...
│   ├── chars.js            캐릭터 13종 목록 (룰렛·사다리·스톱워치 공용)
│   ├── sound.js            효과음 엔진 (Web Audio 합성)
│   └── art/                생성한 그림 (배경·썸네일·스톱워치 부품)
│       └── char/           캐릭터 13종 (배경 지운 투명 PNG)
├── tools/
│   ├── build-assets.py     첨부 사진 -> 앱에서 쓰는 파일 만들기
│   ├── find-teeth.py       이빨화면.jpg 에서 이빨 20개 오려내기
│   ├── build-chars.py      캐릭터/ 원본 -> shared/art/char/
...
- **전역 오염 금지**: `game.js` 는 IIFE 로 감싼다.
  전역은 `Sfx`·`Progress`·`GAMES`·`Art`·`Fx`·`Chars` 뿐.
...
- **룰렛**: 룰렛판은 `KakaoTalk_Photo_2026-08-21-16-36-48.jpeg` 를 본떠 SVG 로 그렸습니다.
  그림을 그대로 쓰지 않은 이유는 둘입니다 — 원본에 찍힌 숫자가 뒤섞여 있고(5 두 개, 8 없음),
  참가자 수(2~10)에 따라 칸 수가 바뀌어야 하기 때문입니다.
  **숫자는 안쪽, 캐릭터는 바깥쪽**에 둡니다. 칸이 바깥으로 갈수록 넓어서,
  큰 캐릭터가 바깥에 있어야 칸 안에 다 들어갑니다.
  칸과 참가자 목록은 언제나 같은 캐릭터를 씁니다 (`game.js` 의 `CAST`).
...
`캐릭터/` 에 원본(검은 바탕이 있는 큰 그림)이 있고,
`shared/art/char/` 에 배경을 지운 240px 정사각형 PNG 가 있습니다.

```bash
python3 tools/build-chars.py     # 캐릭터/ -> shared/art/char/
```

목록은 `shared/chars.js` 한 곳에만 둡니다. 세 게임이 여기서 가져다 씁니다.

- **룰렛** — 13종 중 고정으로 고른 10마리 (`룰렛/game.js` 의 `CAST`)
- **사다리** — 13종 중 고정으로 고른 8마리 (`사다리/game.js` 의 `CAST`).
  위쪽 플레이어와 사다리를 타고 내려가는 말이 **같은 캐릭터**를 씁니다.
- **스톱워치** — 들어올 때마다 `Chars.pick(2)` 로 새로 뽑습니다.

캐릭터를 바꾸려면 각 게임의 `CAST` 배열만 고치면 됩니다.
그림이 없으면 이모지로 물러나므로, 파일이 빠져도 게임은 그대로 돌아갑니다.
