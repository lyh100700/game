...
│   ├── wheel-faces.py      룰렛 그림에서 캐릭터 10개 오려내기
...
- **룰렛**: 칸에 들어가는 캐릭터 10종은 `tools/wheel-faces.py` 가 룰렛 그림에서
  오려낸 `룰렛/faces/f01~f10.png` 입니다. 참가자 목록도 같은 얼굴을 씁니다.
  룰렛판 자체는 원본 그림에 찍힌 숫자가 뒤섞여 있어(5·8 중복) 그림을 그대로 쓰지 않고,
  같은 색·같은 구성으로 SVG 로 다시 그렸습니다. 그래야 참가자 수(2~10)도 바꿀 수 있습니다.
