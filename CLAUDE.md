...
- **위아래 띠**: 주소창·상태바 색(`theme-color`)은 다섯 화면 모두 `#163a2d` 로 같다.
  화면마다 다르면 게임을 옮길 때 띠 색이 깜빡인다.
...
- **효과음**: 오디오 파일을 쓰지 않는다. `shared/sound.js` 에서 Web Audio 로 합성한다.
  새 소리가 필요하면 그 파일의 `SOUNDS` 에 추가한다. 만들 때 지킬 것:
  소리를 0 에서 시작하지 말 것(딱 하는 잡음이 생긴다), 자주 나는 소리는
  칠 때마다 높이를 조금 흔들 것, 한 소리를 여러 겹으로 쌓을 것.
  **`DynamicsCompressor` 는 쓰지 않는다** — 문턱보다 한참 낮은 소리까지 10dB 넘게
  눌러서 앱 전체가 먹먹해진다. 대신 `softClip()` 파형 리미터를 마지막에 둔다.
...
- **사다리·룰렛·스톱워치**: 셋 다 `shared/art/bg-clouds.svg` 파스텔 구름 하늘을 씁니다.
  원본 스크린샷에는 UI 가 함께 찍혀 있어 배경으로 쓸 수 없어 CSS 로 재현한 것입니다.
...
- **스톱워치** — 들어올 때마다 세 마리를 새로 뽑습니다.
  자리도 매번 다릅니다. `game.js` 의 `placeBuddies()` 가 버튼·시계·결과판의 실제
  자리를 재서 빈 곳만 골라 놓으므로, 눌러야 할 것을 가리는 일이 없습니다.
